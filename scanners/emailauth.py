...
_REPORT_COLS = ("campanha,domain,has_mx,mx,spf_status,dmarc_status,"
                "dkim_status,dkim_selector,risk,issues,status")


def _row_to_result(row) -> dict:
    """Converte uma linha de `domains` no dicionário que o relatório espera."""
    (campanha, domain, has_mx, mx, spf_status, dmarc_status,
     dkim_status, dkim_selector, risk, issues, status) = row
    return {
        "campanha": campanha or "", "domain": domain or "",
        "has_mx": bool(has_mx), "mx": mx or "",
        "spf_status": spf_status or "", "dmarc_status": dmarc_status or "",
        "dkim_status": dkim_status or "", "dkim_selector": dkim_selector or "",
        "risk": risk or "INFO", "status": status or "",
        # process_results grava issues como texto " | "; o relatório espera lista.
        "issues": [i for i in (issues or "").split(" | ") if i],
        # O banco guarda o VEREDITO (AUSENTE/NONE/...), não o TXT bruto de
        # SPF/DMARC. Domínios de campanhas que NÃO rodaram nesta execução
        # aparecem com o veredito certo e sem o registro bruto; o que a execução
        # corrente descobriu volta inteiro pelo overlay de load_report_rows().
        "spf_raw": "", "dmarc_raw": "",
    }
...
def load_report_rows(frescos: list[dict] | None = None):
    ...
    banco (só eles têm o TXT bruto de SPF/DMARC) e como CÓPIA, para que o
...
def main():
    ...
    try:
        ...
        # em memória entra por cima para não perder o TXT bruto de SPF/DMARC,
        ...
        if ack is not None:
            _ack_n = ack.apply("email", rep_novos, rep_rein)
            if _ack_n:
        ...
        generate_email_report(rep_novos, rep_rein, rep_rem, output_path=_out)
    ...
    # "Spoofáveis" conta o relatório inteiro (todas as campanhas); as demais
    # contagens são da EXECUÇÃO, que com ARGUS_CAMPANHA cobre só uma campanha.
    spoofaveis = sum(1 for r in rep_novos + rep_rein if r.get("risk") in ("CRITICO", "ALTO"))
    print()
